...
# Define admin ID
ADMIN_ID = 7357694692  # Replace with your admin's Telegram ID
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_message = (
        ...
        "👑 **Bot Owner:** [LOST OP](https://t.me/DEVX76)\n"
...
async def help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_message = (
        ...
        "👨💻 *Developer:* [@DEVX76](https://t.me/DEVX76)\n"
...
async def run_attack(ip, port, user_id, context):
    ...
    try:
        ...
        # Fetch username before logging the attack
        username = await get_user_info(user_id, context)
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = await get_user_info(user_id, context)  # Fetch username
...
async def logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for log in logs_list:
        try:
            ...
            username = log.split("Username: `")[1].split("`")[0].strip()  # Extract username
            target = log.split("Target: `")[1].split("`")[0].strip()
            attack_time = log.split("Time: `")[1].split("`")[0].strip()

            # Format log entry correctly
            log_entry = (
                f"👤 **User ID:** `{user_id}`\n"
                f"👤 **Username:** {username}\n"  # Removed backticks
                f"🎯 **Target:** `{target}`\n"
...
def main():
    application = Application.builder().token("7255031773:AAFTn3H2chqUyTG-n5WoAuquobpCaf3ay_o").build()
